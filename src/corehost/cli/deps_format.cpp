    ...
    const std::function<const vec_asset_t&(const pal::string_t&, int, bool*)>& get_assets_fn)
{
    ...
    for (const auto& library : json[_X("libraries")].GetObject())
    {
        ...
        if (!library_exists_fn(library.name.GetString()))
        {
...
bool deps_json_t::process_runtime_targets(const rapidjson::Value& json, const pal::string_t& target_name, const rid_fallback_graph_t& rid_fallback_graph, rid_specific_assets_t* p_assets)
{
    rid_specific_assets_t& assets = *p_assets;
    for (const auto& package : json[_X("targets")][target_name.c_str()].GetObject())
    {
        const auto& targets = package.value.GetObject();
        if (!targets.HasMember(_X("runtimeTargets")))
        {
            continue;
        }

        for (const auto& file : targets[_X("runtimeTargets")].GetObject())
        {
...
bool deps_json_t::process_targets(const rapidjson::Value& json, const pal::string_t& target_name, deps_assets_t* p_assets)
{
...
bool deps_json_t::load_framework_dependent(const pal::string_t& deps_path, const rapidjson::Value& json, const pal::string_t& target_name, const rid_fallback_graph_t& rid_fallback_graph)
{
...
bool deps_json_t::load_self_contained(const pal::string_t& deps_path, const rapidjson::Value& json, const pal::string_t& target_name)
{
    if (!process_targets(json, target_name, &m_assets))
    {
        return false;
...
bool deps_json_t::load(bool is_framework_dependent, const pal::string_t& deps_path, const rid_fallback_graph_t& rid_fallback_graph)
{
    ...
    if (!m_json.parse_file(deps_path))
    {
        return false;
    }
    ...
    const auto& json = m_json.document();
    const auto& runtime_target = json[_X("runtimeTarget")];
